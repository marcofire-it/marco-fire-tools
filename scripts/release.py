...
def staging_head() -> str:
    # Usiamo HEAD locale come "source di verità" — è quello che vogliamo rilasciare.
    # Se diverge da origin/main, lo segnaliamo ma non blocchiamo (workflow legittimo:
    # rebase + release SENZA force-push intermedio su staging).
    local_head = must(["git", "rev-parse", "HEAD"])
    try:
        origin_head = must(["git", "rev-parse", f"{STAGING_REMOTE}/{BRANCH}"])
        if origin_head != local_head:
            print(
                f"\n[note] HEAD locale ({local_head[:8]}) diverge da {STAGING_REMOTE}/{BRANCH} ({origin_head[:8]}).\n"
                f"       Rilascero' HEAD locale a public. Ricordati di allineare staging dopo:\n"
